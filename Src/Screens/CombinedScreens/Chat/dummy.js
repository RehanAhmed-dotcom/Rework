...
const Chatscreen_busicaramigos = ({ navigation, route }) => {
    ...
    const { SinglePerson } = route.params;
    ...
    const _updateChatCount = async () => {
        try {
            database()
                ...
                .child(resp.userdata.email.replace(/[^a-zA-Z0-9 ]/g, ''))
                .once('value', (snapshot) => {
        ...
        if (type === 'text') {
            if (message) {
                senderMsg(
                    ...
                    resp?.userdata.email.replace(/[^a-zA-Z0-9 ]/g, ''),
                    Date.now(),
                )
                    .then(() => {
                        _chatUsers();
    ...
    const goForFetch = async (voice) => {
        await fetch('https://intechsol.co/bicicita/api/voice', {
            method: 'POST',
            headers: {
    ...
    const uploadImage = async (image) => {
        ...
        await fetch('https://intechsol.co/bicicita/api/upload', {
            method: 'POST',
            headers: {
                'Content-Type': 'multipart/form-data',
    ...
    const _handleMessageNotification = () => {
        ...
        data.append('send_to', resp?.userdata?.id);
        data.append('message', newmessages[0]?.msg);
        data.append('type', 'chat');
        data.append('redirect', '1');
        fetch('https://intechsol.co/bicicita/api/notification-chat', {
            method: 'POST',
            headers: {
                'Content-Type': 'multipart/form-data',
    ...
    const _chatUsers = async () => {
        try {
            ...
            database()
                .ref('users/' + resp?.userdata.email.replace(/[^a-zA-Z0-9 ]/g, ''))
                .child(user?.userdata.email.replace(/[^a-zA-Z0-9 ]/g, ''))
                .once('value', (snapshot) => {
    ...
    return (
        <View style={styles.container}>
                ...
                <View style={{ flexDirection: 'row' }}>
                    <Left>
                        <View style={styles.chatuser}>
                            <Image
                                source={{
                                    uri: resp.userdata.profile_image,
                                }}
            ...
            <View style={styles.bottomcontaiber}>
                <View style={{ flexDirection: 'row' }}>
                    <Left>
